 ...
 * - Checks the number of chat emoticons in one message and won't parse if it is more than bigbang.emoticons.maxChatEmotes
...
if (typeof BigBang === 'undefined') global.BigBang = {};
		...
		switch (Users.ShadowBan.checkBanned(user) && match) {
			case true:
				...
				user.sendTo(room, '|html|' + 
					...
					user.name + '">' + '<b><font color="' + BigBang.hashColor(user.userid) + '">' + Tools.escapeHTML(user.name) + ':</font></b></button> ' + message + '</div>'
				);
				room.update();
...
function loadEmotes() {
	try {
		...
		Object.merge(BigBang.emoticons.chatEmotes, emotes);
	} catch (e) {}
}
		...
		try {
			switch (toId(parts[0])) {
				...
				case 'add':
					...
					if (BigBang.emoticons.chatEmotes[emoteName]) return this.errorReply("ERROR - the emoticon: " + emoteName + " already exists.");
					...
					emotes[emoteName] = BigBang.emoticons.chatEmotes[emoteName] = link;
					saveEmotes();
				...
				case 'delete':
					...
					emoteName = parts[1];
					if (!BigBang.emoticons.chatEmotes[emoteName]) return this.errorReply("ERROR - the emoticon: " + emoteName + " does not exist.");
					delete BigBang.emoticons.chatEmotes[emoteName];
				...
				case 'view':
					if (!this.canBroadcast()) return;
					//if (this.broadcasting) return this.errorReply("ERROR: this command is too spammy to broadcast.  Use / instead of ! to see it for yourself.");
					var name = Object.keys(BigBang.emoticons.chatEmotes);
					emoticons = [];
					var len = name.length;
					while (len--) {
						emoticons.push((BigBang.emoticons.processEmoticons(name[(name.length - 1) - len]) + '&nbsp;' + name[(name.length - 1) - len]));
					}
